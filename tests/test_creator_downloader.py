...
class TestAPIResponseParsing:
    ...
    def test_parse_attachment_list(self):
        ...
        images = [a for a in attachments if os.path.splitext(a['path'])[1].lower() in image_exts]
        
        assert len(images) == 2
        
        # Extract archives
        archive_exts = ['.zip', '.rar', '.7z']
        archives = [a for a in attachments if os.path.splitext(a['path'])[1].lower() in archive_exts]
